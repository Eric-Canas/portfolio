        ...
        for (const entry of entries) {
            ...
            if (showTime) {
                ...
                const finish = new Date(
                    dateFinish !== null ? dateFinish : Date.now()
                );
                const difference = finish.getTime() - start.getTime();
                accumulatedTime += difference;
            }
        }
        if (accumulatedTime === 0) return null;
        const years = Math.floor(accumulatedTime / (1000 * 60 * 60 * 24 * 365));
        const months = Math.floor(
            (accumulatedTime % (1000 * 60 * 60 * 24 * 365)) /
                (1000 * 60 * 60 * 24 * 30)
        );
        const monthsString = intl.formatMessage({
            id: `${INTL_PREFIX}.${months > 1 ? "months" : "month"}`,
        });
        const yearsString = intl.formatMessage({
            id: `${INTL_PREFIX}.${years > 1 ? "years" : "year"}`,
        });
        if (years === 0) {
    ...
    render() {
        ...
        return (
            <>
                ...
                {time !== null && (
                    <Typography {...LIST_CATEGORY_PROPS.TIME}>
                        &nbsp; {time}
                    </Typography>
                )}
            </>
